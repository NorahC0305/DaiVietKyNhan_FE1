...
export default function KhaiNhanMoAn({
  ...
  const totalCharacters = Object.keys(landIdToCharacterMap).length;
  ...
  const hasUnlockedAllCharacters = useMemo(
  ...
  const isCharacterClickable = useCallback(
  ...
  const handleClaim = useCallback(() => {
    if (!hasUnlockedAllCharacters) return;
    setIsFinalModalOpen(true);
  }, [hasUnlockedAllCharacters]);

  const handleCloseFinalModal = useCallback(() => {
    setIsFinalModalOpen(false);
    onClaim("ky-linh-viet-hoa");
  }, [onClaim]);
  ...
  return createPortal(
    <>
          ...
          <div className="p-4">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <div className="flex items-center justify-center flex-col">
                ...
                <h2 className="font-bd-street-sign text-center text-3xl lg:text-4xl text-secondary">
                  CHUYỆN CHƯA KỂ
                </h2>
              </div>
              <div className="relative w-[230px] lg:w-[300px] h-[106px] lg:h-[117px]">
            ...
            <div className="w-full mt-5 flex items-center justify-center">
              <span className="text-center font-semibold text-xl text-secondary w-xl">
                Với mỗi vùng đất được chinh phục, mỗi Kỳ Văn được thu thập, bạn
                sẽ mở khóa được một phần sự thật về biến cố đã xảy ra với Kỳ
                Giới và khai mở vùng đất thứ năm - vùng đất cuối cùng. Hãy hoàn
                thành hành trình để mở khóa toàn bộ câu chuyện nhé!
              </span>
            </div>
            ...
            <div className="relative w-full mt-5 flex items-center justify-center gap-4">
              ...
              {hasUnlockedAllCharacters && (
                <button
                  type="button"
                  aria-label="Mở ấn cuối cùng"
                  onClick={handleClaim}
                  className="absolute lg:bottom-70 bottom-28 lg:right-2 right-0 flex-shrink-0 w-14 h-14 lg:w-16 lg:h-16 transition-transform duration-300 hover:scale-105 cursor-pointer"
                >
            ...
            {/* --- Main Content --- */}
            <ContentKhaiNhanMoAn
              isHidden={isHidden || !isCharacterClickable(an)}
              an={an}
            />
          </div>
        </div>
      ...
      <ModalBackdrop
        ...
        className="relative mx-auto w-[90vw] sm:w-[80vw] max-w-4xl"
